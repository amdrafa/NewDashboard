    ...
    if (request.method === "GET") {
      try {
        ...
        return response
          .status(200)
          .json({
            register_number: userData.data.register_number,
            driver_category: userData.data.driver_category,
            expires_at: userData.data.expires_at,
          });
      } catch (err) {
        console.log('error when calling "driver licence" route. ', err);
